...
class AnswerService {
    ...
    async analyze(question, schemas) {
        ...
        if (env_1.DISABLE_EMBEDDINGS) {
            ...
            const plan = { target: db_name, tables: Array.isArray(parsed.plan?.tables) ? parsed.plan.tables : [], filters: Array.isArray(parsed.plan?.filters) ? parsed.plan.filters : [], };
