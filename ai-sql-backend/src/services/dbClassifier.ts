import { SchemaHints } from "./schemaLoader";
...
export function classifyQuestion(question: string, schemas: { entities: SchemaHints; dms: SchemaHints }): Classification {
  const q = question.toLowerCase();

  let entitiesScore = 0;
  for (const kw of entitiesKeywords) {
    if (q.includes(kw)) entitiesScore += 1;
  }

  let dmsScore = 0;
  for (const kw of dmsKeywords) {
    if (q.includes(kw)) dmsScore += 1;
  }
